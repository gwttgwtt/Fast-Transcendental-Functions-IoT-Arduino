...
int active_iterations = ITERATIONS;  // по подразбиране — всички
...
void processSHOWCommand(int steps) {
  ...
  for (int angle_deg = 0; angle_deg <= 360; angle_deg++) {
    ...
    cordic_sin_cos(angle_deg, cos_out, sin_out, steps);
    unsigned long t2_cordic = micros();
    float sin_c = (float)sin_out / SCALE;
    uint8_t pwm_cordic = cordicToPWM(sin_out);
